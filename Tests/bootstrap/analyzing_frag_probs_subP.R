...
plot150 =
  ...
  facet_grid(
    ...
    drop     = FALSE,
    # scales   ="free", # This doesn't look too good.
    labeller = to_string
  ) +
...
plot300=
  ggplot( data = D4P300$boot %>% rename(Algorithm=algo),
          aes(x=WH, y=prob, fill=Algorithm, color=Algorithm)
  ) +
  geom_hline(aes(yintercept=uniform_prob), linetype="dashed")+
  geom_boxplot() +
  theme_minimal() +
...
big_plot = cowplot::plot_grid(plot150, plot300, nrow=2, align='v', rel_heights = c(1.5,.75) )
